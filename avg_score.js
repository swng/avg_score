...
function toPage(in_field, i) { // for debugging purposes
...
function occupiedCorner(field, corner) { // field.at with extra check for out of bounds
    if (corner[1] < 0 || corner[0] < 0 || corner[0] > 9) return true;
    return field.at(corner[0], corner[1]) != '_';
}

function clearedOffset(rowsCleared, yIndex) { // given previously cleared rows, what is the "global" y index of the piece?
...
function inverse_clearedOffset(rowsCleared, yIndex) { // given previously cleared rows and the global y index, what is the "local" y index?
    offset = 0;
    for (let row of rowsCleared) {
...
function get_score(queue, solution_pages, b2b = true, combo = 1, b2b_end_bonus = 0) {
...
for (let queue in data) {
    if (queue != "sequence" && queue !='') {
                ...
                for (queue_2 of hold_reorderings) {
                    ...
                    if (valid) {
                        ...
                        if (property in memoize) {
                            ...
                            let temp = get_score(queue_2, pages, false, 0, 300);
        ...
        if (max_score == -3000) { // change this if you know you're working with non 100% setups with fail queues
